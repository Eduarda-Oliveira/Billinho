	...
	module V1
        class Api::V1::InstitutionsController < ApplicationController   
            ...
            def index
                ...
                render json: {status: 'SUCCESS', message: 'Institutions de ensino loaded', data:institutions}, status: :ok
            end
			...
			def update
				...
				if institution.update_attributes(institution_params)
					...
					render json: {status: 'ERROR', message:'Institution not update', data:institution.errors},status: :unprocessable_entity
				end
			end
            # parametros aceitos
            private
